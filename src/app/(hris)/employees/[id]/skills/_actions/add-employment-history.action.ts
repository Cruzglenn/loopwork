...
type AddEmploymentHistoryActionState = ActionReturnType<
  Form,
  undefined,
...
export async function addEmploymentHistory(
  prevState: AddEmploymentHistoryActionState,
  formData: FormData,
): Promise<AddEmploymentHistoryActionState> {
  const form: Form = {
  ...
  try {
    await api.employees.addEmploymentHistory(form.employeeId, validationResult.data);

...
import { deleteEquipmentDocument } from '../_actions/delete-equipment-document.action';
...
export function DocumentItemMenu({ document, actions, equipmentId }: Props): JSX.Element {
  ...
  const deleteDocumentAction = deleteEquipmentDocument.bind(null, equipmentId, document.id);
  ...
  const handleAction = async (action: Action, id: string) => {
    switch (action.id) {
      case 'edit':
        router.push(HRIS_ROUTES.equipment.documents.edit(equipmentId, id));
        break;
      case 'open':
  ...
  return (
    <>
      <Menu
        aria-label={tNext('documentItemMenu', {
